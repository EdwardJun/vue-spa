<style lang="sass">
    @import '../sass/page/serviceItemCoupon.scss';
</style>
<template>
    <div class="page" id="service-item-coupon-page">
        <div class="tab-list">
            <div :class="{ active: currTab == index }" v-for="(item, index) in tabList" :key="index" @click="doSwitchTab(index)">{{item}}</div>
        </div>

        <!--3个列表-->
        <div v-for="(dataObj, dataIndex) in dataArr" :key="dataIndex" v-show="currTab==dataIndex" class="list item-content" :ref="'listEle'+dataIndex" :style="{ height : (global.winHeight-2.0833*global.winScale*16)+'px' }" @scroll="doHandlerListScroll()">
            <div class="list-item" v-for="(singleClubData, singleIndex) in dataObj.dataList" :key="singleIndex">
                ...
                <div v-show="!singleClubData.isClosed" class="item-coupon">
                    <template v-for="typeObj in singleClubData.listObj">
                        <coupon-item v-for="(item, itemIndex) in typeObj.list" :key="item.userActId" :class="item.couponStatus==1 ? 'item-data'+item.couponStatus : 'item-data'" :coupon-data="item"></coupon-item>
                    </template>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            doHandlerListScroll () {
                ...
                // console.log('listEle', JSON.stringify(that.$refs['listEle' + that.currTab][0]))
                // [{"_prevClass":"list","__vOriginalDisplay":""}]
                // {"_prevClass":"list","__vOriginalDisplay":""}
                let listEle = that.$refs['listEle' + that.currTab][0]
                // console.log('dataObj', JSON.stringify(dataObj))
                // console.log('listEle', JSON.stringify(listEle))
            ...
            doHandlerData (data, dataObj) {
                ...
                for (let k = 0; k < data.length; k++) {
                    ...
                    // console.log('currCouponType', currCouponType)
            ...
            doSwitchTab (index) {
                ...
                if (that.currTab != index) {
                    ...
                    that.currTab = index
                    that.$router.replace({name: 'serviceItemCoupon', query: {tab: index, all: that.isQueryAll}})
                }
            },
            /**
             * 跳转之前缓存数据
